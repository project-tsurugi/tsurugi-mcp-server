...
public class IceaxeSessionPool extends SessionPool {
    private static final Logger LOG = LoggerFactory.getLogger(IceaxeSessionPool.class);

    public static IceaxeSessionPool create(Arguments arguments) {
        ...
        var sessionOption = TgSessionOption.of();
        {
            ...
            arguments.findDbTimeout().ifPresent(timeout -> sessionOption.setTimeout(TgTimeoutKey.DEFAULT, timeout, TimeUnit.SECONDS));
        }

        var connector = getConnector(endpoint, credentialList, sessionOption);
        LOG.debug("connector={}", connector);

        var pool = new IceaxeSessionPool(connector, sessionOption);
    ...
    private IceaxeSessionPool(TsurugiConnector connector, TgSessionOption sessionOption) {
        this.connector = connector;
        this.sessionOption = sessionOption;
    }

    @Override
    protected TsurugiMcpSession createSession() throws IOException {
        var session = connector.createSession(sessionOption);
        return new IceaxeSession(this, session);
    }
}
